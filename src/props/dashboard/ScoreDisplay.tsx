  ...
  return (
    <Flex>
      <Box shadow={"lg"} rounded={"md"} style={{
        ...
        textAlign: "center",
        lineHeight: "2.5rem",
        backgroundColor: props.color == "red" ? "#F56565" : props.color == "blue" ? "#11B5E4" : props.color == "yellow" ? "#F9A825" : props.color == "green" ? "#48BB78" : "white",
        color: "black",
        width: "20rem",
      }}
      >
        {props.editable ?
          (<Box mt={"1rem"} p={"0"}>
            <Box mt={"1rem"} onClick={() => { setDropDownOpen(!dropDownOpen) }} style={{ cursor: "pointer" }}>
              {props.team?.cname || "未設定"}
              <br />
              {props.team?.ename || "Not Set"}
            </Box>
